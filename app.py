...
from supabets_event_probe import discover_supabets_sport_calls, fetch_supabets_soccer_event_samples, discover_supabets_event_detail_calls, discover_supabets_b2c_betting_paths, discover_supabets_public_routes, discover_supabets_sports_page_calls, probe_supabets_event_endpoint_candidates, inspect_supabets_competition_object, discover_supabets_subevent_loader, discover_supabets_odds_loader_tokens, discover_supabets_sport_api_calls_escaped, inspect_supabets_sport_client_raw, inspect_supabets_sport_client_module, inspect_supabets_eventsprogram_module, inspect_supabets_api_clients, inspect_supabets_fixtures_route_context, inspect_supabets_fixtures_page, inspect_supabets_unique_fixtures_chunks, inspect_supabets_unique_fixture_chunk_deep, inspect_supabets_fixture_chunk_fetch_calls, inspect_supabets_unique_sports_chunks, inspect_supabets_sports_chunk_betting_tokens, inspect_supabets_event_card_click_logic, inspect_supabets_sports_route_literals, inspect_supabets_competition_page, inspect_supabets_competition_embed_logic, rank_supabets_competition_scripts, inspect_supabets_top_ranked_competition_script, inspect_supabets_bitville_loader, inspect_supabets_competition_html_payload, inspect_supabets_competition_html_clues, inspect_supabets_direct_public_odds_candidates, inspect_supabets_live_iframe_route, inspect_supabets_unique_live_iframe_chunks, inspect_supabets_live_chunks_excluding_demo
...
@app.get("/api/supabets-sports-page-calls-test")
...
@app.get("/api/supabets-event-endpoint-candidates-test")
async def supabets_event_endpoint_candidates_test():
    return JSONResponse(await probe_supabets_event_endpoint_candidates())

@app.get("/api/supabets-competition-object-test")
async def supabets_competition_object_test():
    return JSONResponse(await inspect_supabets_competition_object())

@app.get("/api/supabets-subevent-loader-test")
async def supabets_subevent_loader_test():
    return JSONResponse(await discover_supabets_subevent_loader())

@app.get("/api/supabets-odds-loader-test")
async def supabets_odds_loader_test():
    return JSONResponse(await discover_supabets_odds_loader_tokens())

@app.get("/api/supabets-sport-api-calls-test")
async def supabets_sport_api_calls_test():
    return JSONResponse(await discover_supabets_sport_api_calls_escaped())
...
@app.get("/api/supabets-sport-client-module-test")
async def supabets_sport_client_module_test():
    return JSONResponse(await inspect_supabets_sport_client_module())

@app.get("/api/supabets-eventsprogram-module-test")
async def supabets_eventsprogram_module_test():
    return JSONResponse(await inspect_supabets_eventsprogram_module())

@app.get("/api/supabets-api-clients-test")
async def supabets_api_clients_test():
    return JSONResponse(await inspect_supabets_api_clients())

@app.get("/api/supabets-fixtures-route-test")
async def supabets_fixtures_route_test():
    return JSONResponse(await inspect_supabets_fixtures_route_context())

@app.get("/api/supabets-fixtures-page-test")
async def supabets_fixtures_page_test():
    return JSONResponse(await inspect_supabets_fixtures_page())

@app.get("/api/supabets-fixtures-unique-chunks-test")
async def supabets_fixtures_unique_chunks_test():
    return JSONResponse(await inspect_supabets_unique_fixtures_chunks())

@app.get("/api/supabets-fixture-chunk-deep-test")
async def supabets_fixture_chunk_deep_test():
    return JSONResponse(await inspect_supabets_unique_fixture_chunk_deep())

@app.get("/api/supabets-fixture-fetch-calls-test")
async def supabets_fixture_fetch_calls_test():
    return JSONResponse(await inspect_supabets_fixture_chunk_fetch_calls())

@app.get("/api/supabets-sports-unique-chunks-test")
async def supabets_sports_unique_chunks_test():
    return JSONResponse(await inspect_supabets_unique_sports_chunks())

@app.get("/api/supabets-sports-betting-tokens-test")
async def supabets_sports_betting_tokens_test():
    return JSONResponse(await inspect_supabets_sports_chunk_betting_tokens())

@app.get("/api/supabets-event-card-click-test")
async def supabets_event_card_click_test():
    return JSONResponse(await inspect_supabets_event_card_click_logic())

@app.get("/api/supabets-sports-route-literals-test")
async def supabets_sports_route_literals_test():
    return JSONResponse(await inspect_supabets_sports_route_literals())

@app.get("/api/supabets-competition-page-test")
async def supabets_competition_page_test():
    return JSONResponse(await inspect_supabets_competition_page())

@app.get("/api/supabets-competition-embed-test")
async def supabets_competition_embed_test():
    return JSONResponse(await inspect_supabets_competition_embed_logic())

@app.get("/api/supabets-competition-script-rank-test")
async def supabets_competition_script_rank_test():
    return JSONResponse(await rank_supabets_competition_scripts())

@app.get("/api/supabets-top-ranked-script-test")
async def supabets_top_ranked_script_test():
    return JSONResponse(await inspect_supabets_top_ranked_competition_script())

@app.get("/api/supabets-bitville-loader-test")
async def supabets_bitville_loader_test():
    return JSONResponse(await inspect_supabets_bitville_loader())

@app.get("/api/supabets-competition-html-test")
async def supabets_competition_html_test():
    return JSONResponse(await inspect_supabets_competition_html_payload())

@app.get("/api/supabets-competition-html-clues-test")
async def supabets_competition_html_clues_test():
    return JSONResponse(await inspect_supabets_competition_html_clues())

@app.get("/api/supabets-direct-odds-candidates-test")
async def supabets_direct_odds_candidates_test():
    return JSONResponse(await inspect_supabets_direct_public_odds_candidates())

@app.get("/api/supabets-live-iframe-test")
async def supabets_live_iframe_test():
    return JSONResponse(await inspect_supabets_live_iframe_route())

@app.get("/api/supabets-live-unique-chunks-test")
async def supabets_live_unique_chunks_test():
    return JSONResponse(await inspect_supabets_unique_live_iframe_chunks())

@app.get("/api/supabets-live-filtered-chunks-test")
async def supabets_live_filtered_chunks_test():
    return JSONResponse(await inspect_supabets_live_chunks_excluding_demo())
