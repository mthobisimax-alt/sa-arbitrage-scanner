...
def _identity(q):
    return (event_key(q),str(q.get("market_id") or ""),normalize_text(q.get("market_type","")),normalize_text(q.get("period","")),normalize_text(q.get("line","")),int(q.get("market_length") or 0))
...
def _complete_group(best,rows):
    ...
    return len(labels)==expected and all(labels)
...
def find_preferred_arbs(quotes,preferred_bookmakers,max_age=20,min_margin=0.10):
    """Find arbs while forcing each preferred bookmaker quote as one leg."""
    ...
    for rows in _groups(quotes,max_age).values():
        ...
        selections={normalize_text(q.get("selection")) for q in rows if q.get("selection")}
        ...
        for anchor in rows:
            ...
            seen.add(signature); opportunities.append(opp)
    return sorted(opportunities,key=lambda x:x["margin"],reverse=True)
